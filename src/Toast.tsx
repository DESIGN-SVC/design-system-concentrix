...
const Blocked = ({ ...props }: ComponentPropsWithRef<"svg">) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="17"
...
const Check = ({ ...props }: ComponentPropsWithRef<"svg">) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="17"
...
const Warning = ({ ...props }: ComponentPropsWithRef<"svg">) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="17"
...
const Trigger = ({ children, asChild }: TriggerProps) => {
  ...
  return <Comp onMouseDown={() => showToast()}>{children}</Comp>;
};
...
const Root = ({
  ...
  }, [isVisible, duration, removeToast]);

  useEffect(() => {
    if (!isVisible) return;
    if (!isDragging) {
      const toastWidth = toastRef.current?.clientWidth ?? 0;
      const toastHeight = toastRef.current?.clientHeight ?? 0;
      ...
      const isOutOfBounds =
        ...
        (toastPosition?.includes("center") &&
          (positionY >= toastHeight || positionY <= -toastHeight));

      if (isOutOfBounds) removeToast();
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    if (!toastPosition?.includes("center")) {
      ...
      setPositionY((prev) => {
        ...
        if (toastPosition?.includes("bottom"))
          return Math.max(prev + deltaY, 0);
        if (toastPosition?.includes("top")) return Math.min(prev + deltaY, 0);
        return prev + deltaY;
      });
    }
  };
    ...
    >
      <div
        ...
        className={cx(
          ...
          {
            "animate-slide-left": ["bottom-right", "top-right"].includes(
              toastPosition
            ),
            "animate-slide-right": ["bottom-left", "top-left"].includes(
              toastPosition
            ),
            "animate-slide-bottom": toastPosition === "top-center",
            "animate-slide-top ": toastPosition === "bottom-center",
          }
        )}
        style={{
...
const Content = ({
  ...
  return (
    <aside className={cx(styles.root, styles.toastStyle[toastType])} {...props}>
...
const Title = ({ children }: { children: ReactNode }) => (
  <h6 className="font-bold leading-[normal] text-sm mb-2">{children}</h6>
);
...
const Close = () => {
  ...
  return (
    <button className="absolute top-4 right-4" onClick={() => removeToast()}>
      <X />
    </button>
  );
};
...
const Confirm = ({ children, onConfirm }: ConfirmProps) => {
  const { removeToast } = useToast();
  return (
    <button
      className={cx([
        "mt-4 float-end px-6 py-2.5 rounded-full",
